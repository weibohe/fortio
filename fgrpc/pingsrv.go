...
func (s *pingSrv) Ping(c context.Context, in *PingMessage) (*PingMessage, error) {
	...
	if len(in.Delay) > 0 {
		s := time.Duration(generateDelay(in.Delay))
		log.LogVf("GRPC ping: sleeping for %v", s)
		time.Sleep(s)
	}
	if in.ErrorPercent > 0 && shouldInjectError(int(in.ErrorPercent)) {
		log.Infof("GRPC ping: injecting error")
		return nil, status.Errorf(codes.Unavailable, "Injected by fortio")
	}
	return &out, nil
}
...
func PingClientCall(serverAddr, cacert string, n int, payload string, delay string) (float64, error) {
	...
	msg := &PingMessage{Payload: payload, Delay: delay}
